...
public class word_search_solver {
	...
	public static void main(String[] args) {
		...
		// main programm structure, loops through word list and for each one goes
		// through each direction array
		// checking if the word is present in said array. When the word is found in a
		// line of an array, it finds
		// the row and column of the first letter of the word in the matrix. The method
		// varies for each direction.
		// Afterwards a word object is created with the word info and added to
		// found_words array.
		loop: for (String word : words) {
			for (String s : horz) {
				if (s.contains(word)) {
					...
					if (!isPalindrome(word))
						continue loop; // if it isn't a palindrome, there is no need to keep searching
				}
			}
			for (String s : vert) {
			...
			for (String s : diag_dr) {
				if (s.contains(word)) {
					...
					int col = dcol + s.indexOf(word); // row and column of word
			...
			for (String s : diag_ul) {
				if (s.contains(word)) {
					...
					int dcol = idx > sopa.getSize() ? (2 * sopa.getSize()) - idx : sopa.getSize(); // column of diagonal
																									// start
					int row = drow - s.indexOf(word);
					int col = dcol - s.indexOf(word); // row and column of word
